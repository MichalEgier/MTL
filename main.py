...
class Neural_Network:
    ...
    def _actualise_weights(self, deltas, a_vectors):
        for i in range(len(self.weights)):
            self.weights[i] += self.learning_coeff/self.batch_size * deltas[i].dot(a_vectors[i].T)

    def _actualise_biases(self, deltas):
        for i in range(len(self.biases)):
            self.biases[i] += self.learning_coeff/self.batch_size * np.array([deltas[i].sum(axis=1)]).T
...
def run(network, number_of_epochs):
